 ...
 *
 */
open class ConfirmDialog protected constructor(
    context: Context,
    themeResId: Int = R.style.ECCommonDialogStyle
) : AlertDialog(
    context,
    themeResId
) {
    ...
    private fun findViews(inflater: LayoutInflater): View {
        val view: View = inflater.inflate(R.layout.ec_dialog_confirm_layout, null)
    ...
    fun setOk(ok: CharSequence?) {
        if (okView != null) {
            okView!!.text = if (TextUtils.isEmpty(ok)) context.getString(R.string.ec_sure) else ok
        } else {
            this.ok = ok
        }
    }
