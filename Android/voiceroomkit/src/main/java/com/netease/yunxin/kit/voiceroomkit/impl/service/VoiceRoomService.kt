...
internal class VoiceRoomService {
    ...
    fun joinRoom(roomUuid: String, role: String, userName: String, avatar: String?, callback: NECallback2<Unit>) {
                            ...
                            joinChatroomChannel(object : NECallback2<Unit>() {
                                ...
                                override fun onError(code: Int, message: String?) {
                                    ...
                                    leaveRtcChannel(object : NECallback2<Unit>() {
                                        override fun onSuccess(data: Unit?) {
                                            VoiceRoomLog.d(TAG, "leaveRtcChannel roomUuid = $roomUuid success")
                                        }
    ...
    private fun removeListener() {
        ...
        seatListener?.apply { currentRoomContext?.seatController?.removeSeatListener(seatListener!!) }
    }
    ...
    fun getSeatInfo(callback: NECallback2<NESeatInfo>) {
        currentRoomContext?.seatController?.getSeatInfo(callback) ?: callback.onError(NEErrorCode.FAILURE, "roomContext is null")
    }

    fun getSeatRequestList(callback: NECallback2<List<NESeatRequestItem>>) {
        currentRoomContext?.seatController?.getSeatRequestList(callback) ?: callback.onError(NEErrorCode.FAILURE, "roomContext is null")
    }

    fun sendSeatInvitation(seatIndex: Int, user: String, callback: NECallback2<Unit>) {
        currentRoomContext?.seatController?.sendSeatInvitation(seatIndex, user, callback) ?: callback.onError(NEErrorCode.FAILURE, "roomContext is null")
    }

    fun submitSeatRequest(seatIndex: Int, exclusive: Boolean, callback: NECallback2<Unit>) {
        currentRoomContext?.seatController?.submitSeatRequest(seatIndex, exclusive, callback) ?: callback.onError(NEErrorCode.FAILURE, "roomContext is null")
    }

    fun cancelSeatRequest(callback: NECallback2<Unit>) {
        currentRoomContext?.seatController?.cancelSeatRequest(callback) ?: callback.onError(NEErrorCode.FAILURE, "roomContext is null")
    }

    fun leaveSeat(callback: NECallback2<Unit>) {
        currentRoomContext?.seatController?.leaveSeat(callback) ?: callback.onError(NEErrorCode.FAILURE, "roomContext is null")
    }

    fun approveSeatRequest(user: String, callback: NECallback2<Unit>) {
        currentRoomContext?.seatController?.approveSeatRequest(user, callback) ?: callback.onError(NEErrorCode.FAILURE, "roomContext is null")
    }

    fun rejectSeatRequest(user: String, callback: NECallback2<Unit>) {
        currentRoomContext?.seatController?.rejectSeatRequest(user, callback) ?: callback.onError(NEErrorCode.FAILURE, "roomContext is null")
    }

    fun kickSeat(user: String, callback: NECallback2<Unit>) {
        currentRoomContext?.seatController?.kickSeat(user, callback) ?: callback.onError(NEErrorCode.FAILURE, "roomContext is null")
    }

    fun openSeats(seatIndices: List<Int>, callback: NECallback2<Unit>) {
        currentRoomContext?.seatController?.openSeats(seatIndices, callback) ?: callback.onError(NEErrorCode.FAILURE, "roomContext is null")
    }

    fun closeSeats(seatIndices: List<Int>, callback: NECallback2<Unit>) {
        currentRoomContext?.seatController?.closeSeats(seatIndices, callback) ?: callback.onError(NEErrorCode.FAILURE, "roomContext is null")
    }
    ...
    fun setEffectVolume(effectId: Int, volume: Int): Int {
        ...
        val playbackResult = currentRoomContext?.rtcController?.setEffectPlaybackVolume(effectId, volume) ?: NEErrorCode.FAILURE
        if (sendResult == NEErrorCode.SUCCESS && playbackResult == NEErrorCode.SUCCESS) {
            effectVolume = volume
            return NEErrorCode.SUCCESS
        }
    ...
    private fun addRoomListener() {
        roomListener = object : RoomListenerWrapper() {
            ...
            override fun onMemberPropertiesChanged(
                ...
                if (properties.containsKey(MemberPropertyConstants.MUTE_VOICE_KEY)) {
                    ...
                    if (voiceValue == MemberPropertyConstants.MUTE_VOICE_VALUE_ON || voiceValue == MemberPropertyConstants.MUTE_VOICE_VALUE_OFF) {
                        ...
                        VoiceRoomLog.d(TAG, "onMemberAudioMuteChanged voiceRoomMember:$voiceRoomMember,mute:$mute,operateBy:" + getLocalMember())
                        listeners.forEach {
                    ...
                    val voiceRoomMember = mapMember(member)
                    VoiceRoomLog.d(TAG, "onMemberAudioBanned voiceRoomMember:$voiceRoomMember,banned:$banned")
                    listeners.forEach {
            ...
            override fun onRtcAudioOutputDeviceChanged(device: NEAudioOutputDevice) {
                val outputDevice =
                    NEVoiceRoomAudioOutputDevice.fromValue(device.name.uppercase(Locale.getDefault()))
    ...
    private fun addSeatListener() {
        seatListener = object : NESeatEventListener() {
            ...
            override fun onSeatInvitationAccepted(seatIndex: Int, user: String, isAutoAgree: Boolean) {
                VoiceRoomLog.d(TAG, "onSeatInvitationAccepted seatIndex = $seatIndex user = $user isAutoAgree = $isAutoAgree")
                listeners.forEach {
            ...
            override fun onSeatListChanged(seatItems: List<NESeatItem>) {
                ...
                listeners.forEach {
                    it.onSeatListChanged(seatItems.map { neSeatItem -> VoiceRoomUtils.voiceRoomSeatItem2NEVoiceRoomSeatItem(neSeatItem) })
                }
            }
