...
@property(nonatomic, strong) NSMutableArray<NEVoiceRoomUIAlertAction *> *actions;

@end

@implementation NEVoiceRoomUIAlertView

- (instancetype)initWithActions:(NSMutableArray<NEVoiceRoomUIAlertAction *> *)actions {
  if (self = [super init]) {
    _actions = actions;
  }
  return self;
...
- (void)showWithTypes:(NSArray<NSNumber *> *)types info:(id)info {
  ...
  for (NSNumber *typeNum in types) {
    ...
    for (NEVoiceRoomUIAlertAction *model in _actions) {
      if (model.type == type) {
  ...
  if (alertActionModels.count != 0) {
    ...
    [NEUIActionSheet showWithDesc:nil
                     actionModels:alertActionModels
                           action:^(NEUIActionSheetModel *model) {
                             int index = model.sheetId;
                             NEVoiceRoomUIAlertAction *action = weakSelf.actions[index];
                             if (action.handle) {
                               action.handle(info);
                             }
                           }
...
- (UIAlertAction *)alertActionWithModel:(NEVoiceRoomUIAlertAction *)model info:(id)info {
...
@implementation NEVoiceRoomUIAlertAction

+ (NEVoiceRoomUIAlertAction *)actionWithTitle:(NSString *)title
                                         type:(NEUIAlertActionType)type
                                      handler:(NEUIAlertActionHandle)handle {
  NEVoiceRoomUIAlertAction *ret = [[NEVoiceRoomUIAlertAction alloc] init];
  ret.title = title;
  ret.type = type;
  ret.handle = handle;
  return ret;
