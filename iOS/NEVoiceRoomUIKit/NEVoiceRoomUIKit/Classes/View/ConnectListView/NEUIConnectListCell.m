...
@implementation NEUIConnectListCell
...
+ (NEUIConnectListCell *)cellWithTableView:(UITableView *)tableView
  ...
  if ([datas count] > indexPath.row) {
    ...
    NEUIConnectListCell *cell =
        [tableView dequeueReusableCellWithIdentifier:[NEUIConnectListCell description]];
    [cell _loadData:data indexPath:indexPath];
    return cell;
  }
  return [NEUIConnectListCell new];
}
- (void)awakeFromNib {
  [super awakeFromNib];
  // Initialization code
