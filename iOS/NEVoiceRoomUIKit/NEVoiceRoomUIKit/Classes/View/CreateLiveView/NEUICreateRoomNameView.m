...
@interface NEUICreateRoomNameView () <UITextViewDelegate>
...
@implementation NEUICreateRoomNameView
...
- (void)createRandomRoomName {
  [NEVoiceRoomKit.getInstance
      getCreateRoomDefaultInfo:^(NSInteger code, NSString *_Nullable msg,
        ...
        if (code != 0) {
          [NEVoiceRoomToast showToast:NELocalizedString(@"获取房间随机主题失败")];
          return;
        }
...
- (UIButton *)chatRoomButton {
  if (!_chatRoomButton) {
    ...
    [_chatRoomButton setTitle:NELocalizedString(@"语音聊天室") forState:UIControlStateNormal];
    [_chatRoomButton setImage:[NEVoiceRoomUI ne_imageName:@"chatroom_titleIcon"]
...
- (UIButton *)ktvButton {
  if (!_ktvButton) {
    ...
    [_ktvButton setImage:[NEVoiceRoomUI ne_imageName:@"ktv_titleIcon"]
                forState:UIControlStateNormal];
...
- (UIView *)divideView {
  if (!_divideView) {
    _divideView = [NEUIViewFactory createViewFrame:CGRectZero
...
- (UIView *)slideView {
  if (!_slideView) {
    _slideView = [NEUIViewFactory createViewFrame:CGRectZero BackgroundColor:HEXCOLOR(0x337EFF)];
  }
  return _slideView;
}
