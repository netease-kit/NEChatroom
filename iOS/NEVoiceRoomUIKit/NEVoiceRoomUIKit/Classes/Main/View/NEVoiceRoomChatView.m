...
@implementation NEVoiceRoomChatViewMessage
...
@interface NEVoiceRoomChatCell () {
  CGRect _preRect;
}

/// 消息模型
@property(nonatomic, strong) NEVoiceRoomChatViewMessage *model;
/// 富文本控件
@property(nonatomic, strong) M80AttributedLabel *attributedLabel;

@end

@implementation NEVoiceRoomChatCell
...
- (void)installWithModel:(NEVoiceRoomChatViewMessage *)model indexPath:(NSIndexPath *)indexPath {
...
+ (NEVoiceRoomChatCell *)cellWithTableView:(UITableView *)tableView
  ...
  NEVoiceRoomChatCell *cell =
      [tableView dequeueReusableCellWithIdentifier:[NEVoiceRoomChatCell description]];
  if ([datas count] > indexPath.row) {
    NEVoiceRoomChatViewMessage *model = datas[indexPath.row];
...
@interface NEVoiceRoomChatView () <UITableViewDelegate, UITableViewDataSource> {
  CGRect _preRect;
}

@property(nonatomic, strong) UITableView *tableView;

@property(nonatomic, strong) NSMutableArray<NEVoiceRoomChatViewMessage *> *messages;
///缓存的插入消息,聊天室需要在另外个线程计算高度,减少UI刷新
@property(nonatomic, strong) NSMutableArray<NEVoiceRoomChatViewMessage *> *pendingMessages;

@end

@implementation NEVoiceRoomChatView
...
- (UITableView *)tableView {
  if (!_tableView) {
    ...
    [_tableView registerClass:[NEVoiceRoomChatCell class]
        forCellReuseIdentifier:[NEVoiceRoomChatCell description]];
...
- (void)caculateHeight:(NSArray<NEVoiceRoomChatViewMessage *> *)messages {
...
- (void)processPendingMessages {
  ...
  for (NEVoiceRoomChatViewMessage *message in insert) {
...
- (void)addModels:(NSArray<NEVoiceRoomChatViewMessage *> *)models {
...
- (void)scrollToBottom:(NSArray<NEVoiceRoomChatViewMessage *> *)newModels {
  //    UIEdgeInsets insets = self.tableView.contentInset;
  //    if (insets.top != 0) {
  //        CGFloat height = 0;
  //        for (NEVoiceRoomChatViewMessage *model in newModels) {
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
  return [NEVoiceRoomChatCell heightWithIndexPath:indexPath datas:self.messages];
}

- (UITableViewCell *)tableView:(UITableView *)tableView
         cellForRowAtIndexPath:(NSIndexPath *)indexPath {
  return [NEVoiceRoomChatCell cellWithTableView:tableView indexPath:indexPath datas:self.messages];
}
