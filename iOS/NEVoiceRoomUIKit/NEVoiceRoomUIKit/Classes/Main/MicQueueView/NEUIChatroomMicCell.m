...
+ (CGFloat)cellPaddingH {
  return [NEVoiceRoomUI seatLineSpace];
}

+ (CGFloat)cellPaddingW {
  return [NEVoiceRoomUI seatItemSpace];
}
...
- (NEUIAnimationButton *)connectBtn {
  if (!_connectBtn) {
    ...
    UIImage *img = [UIImage nevoiceRoom_imageNamed:@"mic_none_ico"];
...
- (UIImageView *)singIco {
  if (!_singIco) {
    UIImage *img = [UIImage nevoiceRoom_imageNamed:@"mic_sing_ico"];
