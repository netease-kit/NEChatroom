...
extension CAShapeLayer {
  ...
  @nonobjc
  func addAnimations(for shape: ShapeItem,
...
extension Trim {
    ...
    -> (strokeStart: KeyframeGroup<LottieVector1D>, strokeEnd: KeyframeGroup<LottieVector1D>, pathMultiplier: PathMultiplier)
  {
    ...
    let startStrokes = adjustedStrokeStart.keyframes.map(\.value.cgFloatValue)
    let endStrokes = adjustedStrokeEnd.keyframes.map(\.value.cgFloatValue)
  ...
  private func startValueIsAlwaysGreaterThanEndValue() -> Bool {
    let keyframeTimes = Set(start.keyframes.map(\.time) + end.keyframes.map(\.time))
  ...
  private func adjustKeyframesForTrimOffsets(strokeKeyframes: ContiguousArray<Keyframe<LottieVector1D>>,
