...
extension GradientRenderLayer {
  ...
  func addGradientAnimations(for gradient: GradientShapeItem,
    ...
    try addAnimation(
      ...
      value: { colorComponents in
        gradient.colorConfiguration(from: colorComponents, type: type).map(\.color)
      },
      context: context
    )
    ...
    try addAnimation(
      ...
      keyframes: gradient.colors.keyframes,
      value: { colorComponents in
        gradient.colorConfiguration(from: colorComponents, type: type).map(\.location)
      },
      context: context
    )
  ...
  private func addLinearGradientAnimations(for gradient: GradientShapeItem,
