...
let NE = NEVoiceRoomNetwork.shared

/// 网络请求类
internal class NEVoiceRoomNetwork {
  let tag: String = "NEVoiceRoomNetwork"
  static let shared = NEVoiceRoomNetwork()
  // 配置信息
  ...
  func request(_ url: String,
    ...
    NEVoiceRoomLog.networkLog(
      tag,
      desc: "HTTPRequest\nUrl: \(url).\nMethod: \(method)\nHeaders: \(headers)\nBody: \(body?.prettyJSON ?? "")"
    )
    guard let URL = URL(string: url) else {
      NEVoiceRoomLog.errorLog(tag, desc: "\(url). Bad Url.")
      failed(makeError(NEVoiceRoomErrorCode.failed, "Bad url"))
      return
    }
    ...
    let dataTask = session.dataTask(with: request) { [weak self] data, response, error in
      ...
      guard httpResp.statusCode == 200 else {
        ...
        failed(makeError(NEVoiceRoomErrorCode.failed,
                         "Http error with status: \(httpResp.statusCode)"))
        return
      }
      ...
      guard let response = try? JSONSerialization.jsonObject(
        ...
        failed(makeError(NEVoiceRoomErrorCode.failed, "Cannot decode content data"))
        return
      }
      // 打印requestId
      ...
      NEVoiceRoomLog.infoLog(self.tag, desc: "Response data:\n\(response.prettyJSON)")
      success(dic)
    }
    dataTask.resume()
  }
  ...
  func request<T>(_ url: String,
    ...
    request(url, method: method, headers: headers, body: body) { dic in
      ...
      success(NEVoiceRoomDecoder.decode(returnType, param: dic) ?? nil)
    } failed: { error in
      failed(error)
    }
  }
