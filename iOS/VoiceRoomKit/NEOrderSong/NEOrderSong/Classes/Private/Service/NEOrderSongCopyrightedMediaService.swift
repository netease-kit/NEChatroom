...
let copyrightedTag = "NEOrderSongCopyrightedMediaService"
let maxCopyrightedRetry = 10

class NEOrderSongCopyrightedMediaService: NSObject {
  /// 当前重试幂
  ...
  // 幂等网络请求子线程处理
  func getSongDynamicTokenUntilSuccess(success: ((NEOrderSongDynamicToken?) -> Void)? = nil) {
    NEOrderSong.getInstance().getSongToken { code, msg, dynamicToken in
      if code != 0 {
        NEOrderSongLog.errorLog(copyrightedTag, desc: "获取动态token失败")
        ...
        NEOrderSongLog.successLog(copyrightedTag, desc: "获取动态token成功")
        self.retryLater = 2
  ...
  @objc func timeEvent() {
    NEOrderSongLog.infoLog(copyrightedTag, desc: "动态token即将过期，重新Token")
