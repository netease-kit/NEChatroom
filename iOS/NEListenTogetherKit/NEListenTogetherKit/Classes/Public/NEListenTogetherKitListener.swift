...
public protocol NEListenTogetherListener: NSObjectProtocol {
  @objc
  /// 成员加入聊天室回调
  /// - Parameter members: 成员信息列表
  ///
  /// 添加监听后，有成员加入聊天室会触发此方法
  optional func onMemberJoinChatroom(_ members: [NEListenTogetherMember])

  /// 成员进入房间回调
  /// - Parameter members: 成员列表
  @objc optional func onMemberJoinRoom(_ members: [NEListenTogetherMember])

  /// 成员离开房间回调
  /// - Parameter members: 成员列表
  @objc optional func onMemberLeaveRoom(_ members: [NEListenTogetherMember])

  /// 房间结束回调
  /// - Parameter reason: 房间结束原因
  @objc optional func onRoomEnded(_ reason: NEListenTogetherEndReason)
  ...
  @objc optional func onMemberAudioMuteChanged(_ member: NEListenTogetherMember,
                                               mute: Bool,
                                               operateBy: NEListenTogetherMember?)

  /// 聊天室消息回调
  /// - Parameter message: 文本消息
  ///
  /// 添加监听后，收到聊天室消息会触发此方法
  @objc optional func onReceiveTextMessage(_ message: NEListenTogetherChatTextMessage)

  /// 接收到合唱回调
  /// - Parameters:
  ///   - actionType: 合唱状态
  ///   - songModel: 消息模型
  @objc optional func onReceiveChorusMessage(_ actionType: NEListenTogetherChorusActionType,
                                             songModel: NEListenTogetherSongModel)

  /// 接收到发送礼物的回调
  /// - Parameter giftModel: 礼物模型
  @objc optional func onReceiveGift(giftModel: NEListenTogetherGiftModel)
  /// 当前歌曲播放进度
  /// - Parameters:
  ///   - postion: 歌曲播放进度
  @objc optional func onSongPlayingPosition(_ postion: UInt64)

  @objc optional func onReceiveSongPosition(_ actionType: NEListenTogetherCustomAction,
                                            data: [String: Any]?)

//  /// 音频帧数据
//  /// - Parameter frame: 音频帧数据
//  @objc optional func onRecordingAudioFrame(frame: NEListenTogetherAudioFrame)
  ...
  ///   - member: 成员
  ///   - banned: 是否被禁用音频
  @objc optional func onMemberAudioBanned(_ member: NEListenTogetherMember, banned: Bool)
  /// 麦位变更通知
  /// - Parameter seatItems: 麦位列表
  @objc optional func onSeatListChanged(_ seatItems: [NEListenTogetherSeatItem])
  ...
  /// 点歌
  /// - Parameter song: 歌曲
  @objc optional func onSongOrdered(_ song: NEListenTogetherOrderSongModel?)

  /// 已点列表的删除
  /// - Parameter song: 歌曲
  @objc optional func onSongDeleted(_ song: NEListenTogetherOrderSongModel?)

  /// 已点列表的置顶
  /// - Parameter song: 歌曲
  @objc optional func onSongTopped(_ song: NEListenTogetherOrderSongModel?)

  /// 切歌
  /// - Parameter song: 被切歌曲
  @objc optional func onNextSong(_ song: NEListenTogetherOrderSongModel?)
}
