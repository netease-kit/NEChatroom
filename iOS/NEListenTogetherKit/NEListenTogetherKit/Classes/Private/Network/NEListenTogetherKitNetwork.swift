...
let NE = NEListenTogetherNetwork.shared

/// 网络请求类
internal class NEListenTogetherNetwork {
  let tag: String = "NEListenTogetherNetwork"
  static let shared = NEListenTogetherNetwork()
  // 配置信息
  ...
  func request(_ url: String,
    ...
    NEListenTogetherLog.networkLog(
      tag,
      desc: "HTTPRequest\nUrl: \(url).\nMethod: \(method)\nHeaders: \(headers)\nBody: \(body?.prettyJSON ?? "")"
    )
    guard let URL = URL(string: url) else {
      NEListenTogetherLog.errorLog(tag, desc: "\(url). Bad Url.")
      failed(makeError(NEListenTogetherErrorCode.failed, "Bad url"))
      return
    }
    ...
    let dataTask = session.dataTask(with: request) { [weak self] data, response, error in
      ...
      if let error = error as NSError? {
        ...
        failed(makeError(NEListenTogetherErrorCode.failed, "网络连接失败，请检查你的网络连接！"))
        return
      }
      ...
      guard httpResp.statusCode == 200 else {
        ...
        failed(makeError(NEListenTogetherErrorCode.failed,
                         "Http error with status: \(httpResp.statusCode)"))
        return
      }
      ...
      guard let response = try? JSONSerialization.jsonObject(
        ...
        failed(makeError(NEListenTogetherErrorCode.failed, "Cannot decode content data"))
        return
      }
      // 打印requestId
      if let requestId = response["requestId"] as? String {
        NEListenTogetherLog.infoLog(self.tag, desc: "RequestId:\n\(requestId)")
      }
      guard let code = response["code"] as? Int else {
        failed(makeError(NEListenTogetherErrorCode.failed, "Empty code in response body!"))
        return
      }
      guard code == 0 else {
      ...
      NEListenTogetherLog.infoLog(self.tag, desc: "Response data:\n\(response.prettyJSON)")
      success(dic)
    }
    dataTask.resume()
  }
  ...
  func request<T>(_ url: String,
    ...
    request(url, method: method, headers: headers, body: body) { dic in
      ...
      success(NEListenTogetherDecoder.decode(returnType, param: dic) ?? nil)
    } failed: { error in
      failed(error)
    }
  }
