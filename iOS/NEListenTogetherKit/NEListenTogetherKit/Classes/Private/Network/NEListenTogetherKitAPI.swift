...
enum NEAPI {
  static let prePath =
    "/scene/apps/\(NEListenTogetherKit.getInstance().config?.appKey ?? "")/ent/live/v1"
  // 房间模块
  enum Room {
  ...
  enum Music {
    static let pre_path =
      "/scene/apps/\(NEListenTogetherKit.getInstance().config?.appKey ?? "")/ent/listen/v1"
  ...
  enum PickSong {
    static let pre_path = "/scene/apps/\(NEListenTogetherKit.getInstance().config?.appKey ?? "")/ent/song"
