...
@objc
open class NEListenTogetheRoomInfo: NSObject {
  /// 房间唯一标识
  @objc
  ...
  /// 房间类型
  @objc
  public var roomType: NEListenTogetherLiveRoomType = .multiAudio
  ...
  init(dictionary: [AnyHashable: Any]) {
    ...
    if let liveHostRecordDic = dictionary["liveHostRecord"] as? [AnyHashable: Any] {
      ...
      if let roomTypeRaw = liveHostRecordDic["type"] as? Int {
        roomType = NEListenTogetherLiveRoomType(rawValue: roomTypeRaw) ?? .multiAudio
      }
