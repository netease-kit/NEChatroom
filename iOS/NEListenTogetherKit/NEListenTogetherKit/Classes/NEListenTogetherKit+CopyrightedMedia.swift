...
extension NEListenTogetherKit: NECopyrightedEventHandler, NESongPreloadProtocol {
  public func onTokenExpired() {
    guard let eventHandler = copyrightedEventMediaHandler as? NEListenTogetherCopyrightedMediaEventHandler
    else {
      return
    }
    NEListenTogetherKit.getInstance().copyrightedMediaService?
      .getSongDynamicTokenUntilSuccess(success: { dynamicToken in
        ...
        NEListenTogetherKit.getInstance().copyrightedMediaService?.calculateExpiredTime(timeExpired: expiresIn)
      })
  ...
  public func onPreloadStart(_ songId: String, channel: SongChannel) {
    for pointerListener in preloadProtocolListeners.allObjects {
      guard pointerListener is NEListenTogetherCopyrightedMediaListener, let listener = pointerListener as? NEListenTogetherCopyrightedMediaListener else { continue }
      if listener
        .responds(to: #selector(NEListenTogetherCopyrightedMediaListener
            .voiceroom_onPreloadStart(_:channel:))) {
        listener.voiceroom_onPreloadStart?(songId, channel: channel)
      }
    }
  }
  ...
  public func onPreloadProgress(_ songId: String, channel: SongChannel, progress: Float) {
    for pointerListener in preloadProtocolListeners.allObjects {
      ...
      if listener
        .responds(to: #selector(NEListenTogetherCopyrightedMediaListener
            .voiceroom_onPreloadProgress(_:channel:progress:))) {
        listener.voiceroom_onPreloadProgress?(songId, channel: channel, progress: progress)
      }
    }
  }
  ...
  public func onPreloadComplete(_ songId: String, channel: SongChannel, error: Error?) {
    for pointerListener in preloadProtocolListeners.allObjects {
      ...
      if listener
        .responds(to: #selector(NEListenTogetherCopyrightedMediaListener
            .voiceroom_onPreloadComplete(_:channel:error:))) {
        listener.voiceroom_onPreloadComplete?(songId, channel: channel, error: error)
      }
    }
  }
  ...
  public func setCopyrightedMediaEventHandler(_ eventHandler: NEListenTogetherCopyrightedMediaEventHandler?) {
    NEListenTogetherLog.apiLog(kitTag, desc: "Add copyrighted event handle.")
    guard let eventHandler = eventHandler else {
      NEListenTogetherLog.apiLog(kitTag, desc: "Add copyrighted event handle nil.")
      return
    }
  ...
  public func preloadSong(_ songId: String, channel: SongChannel,
                          observe: NEListenTogetherCopyrightedMediaListener?) {
    guard let observe = observe else {
      NECopyrightedMedia.getInstance().preloadSong(songId, channel: channel, observe: nil)
      return
    }
    NEListenTogetherLog.apiLog(kitTag, desc: "Add copyrighted listener.")
    if !preloadProtocolListeners.allObjects.contains(where: { item in
      if let item = item as? NEListenTogetherCopyrightedMediaListener,
