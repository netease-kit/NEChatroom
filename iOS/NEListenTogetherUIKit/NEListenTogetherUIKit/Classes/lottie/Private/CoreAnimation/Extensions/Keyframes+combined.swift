...
enum Keyframes {
  ...
  ///  - If all of the `KeyframeGroup`s have the exact same animation timing, the keyframes are merged
  ///  - Otherwise, the keyframes are manually interpolated at each frame in the animation
  static func combined<T1, T2, CombinedResult>(_ k1: KeyframeGroup<T1>,
  ...
  ///  - If all of the `KeyframeGroup`s have the exact same animation timing, the keyframes are merged
  ///  - Otherwise, the keyframes are manually interpolated at each frame in the animation
  static func combined<T1, T2, T3, CombinedResult>(_ k1: KeyframeGroup<T1>,
                                                   _ k2: KeyframeGroup<T2>,
  ...
  ///  - If all of the `KeyframeGroup`s have the exact same animation timing, the keyframes are merged
  ///  - Otherwise, the keyframes are manually interpolated at each frame in the animation
  static func combined<T1, T2, T3, T4, T5, T6, T7, CombinedResult>(_ k1: KeyframeGroup<T1>,
                                                                   _ k2: KeyframeGroup<T2>,
                                                                   _ k3: KeyframeGroup<T3>,
  ...
  private static func combined<CombinedResult>(_ allGroups: [AnyKeyframeGroup],
                                               makeCombinedResult: ([Any]) -> CombinedResult?)
    -> KeyframeGroup<CombinedResult> {
    let untypedGroups = allGroups.map(\.untyped)
  ...
  private static func manuallyInterpolated<CombinedResult>(_ allGroups: [AnyKeyframeGroup],
                                                           makeCombinedResult: ([Any]) -> CombinedResult?)
    -> KeyframeGroup<CombinedResult> {
    let untypedGroups = allGroups.map(\.untyped)
    let untypedInterpolators = allGroups.map(\.interpolator)

    let times = untypedGroups.flatMap { $0.keyframes.map(\.time) }
