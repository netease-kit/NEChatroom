...
extension AppDelegate {
  ...
  public func loginRoom(shouldInit: Bool = false) {
    ...
    func login() {
      NEVoiceRoomUIManager.sharedInstance().login(withAccount:accountId, token: accessToken, nickname: nickName) { code, msg, _ in
        if code == 0 {
          NEOrderSong.getInstance().loginInitConfig(accountId, token: accessToken)
